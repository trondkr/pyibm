    ...
    if log is True:
        ...
        for var in varlist:
            ...
            if var in ["taux","tauy","ssh"]:
                ...
                kXYZ+=1
    print grdSTATION.data.shape, grdSTATION.dataXY.shape

def getData(julian,julianIndex,julianFileA,julianFileB,dz1,dz2,depthIndex1,depthIndex2,grdSTATION,event):
    # DEPRECATED: USE IO.f90 instead: Used to be called using:
    #if event != "ESM RUN":
    ...
    #else:
    #    Tdata,Sdata,NH4SMdata,NH4LGdata,NO3SMdata,NO3LGdata,CHLAdata,windX,windY=IOnetcdf.getData(julian,julianIndex,julianFileA,
    #                                      julianFileB,dz1,dz2,depthIndex1,
    #                                      depthIndex2,grdSTATION,event)
    """Calculate weights to use on input data from file"""
    dwB = np.abs(julian) - np.abs(julianFileA)
    dwA = np.abs(julianFileB) - np.abs(julian)
    ...
    if event != "ESM RUN":
        ...
        Sdata=((grdSTATION.data[julianIndex,depthIndex1,1])*
            ...
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex2,1])*
            (dwB/(dwA+dwB)))*dz2
        NH4SMdata=((grdSTATION.data[julianIndex,depthIndex1,2])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex1,2])*
            (dwB/(dwA+dwB)))*dz1 +((grdSTATION.data[julianIndex,depthIndex2,2])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex2,2])*
            (dwB/(dwA+dwB)))*dz2
        NH4LGdata=((grdSTATION.data[julianIndex,depthIndex1,3])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex1,3])*
            (dwB/(dwA+dwB)))*dz1 +((grdSTATION.data[julianIndex,depthIndex2,3])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex2,3])*
            (dwB/(dwA+dwB)))*dz2
        NO3SMdata=((grdSTATION.data[julianIndex,depthIndex1,4])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex1,4])*
            (dwB/(dwA+dwB)))*dz1 +((grdSTATION.data[julianIndex,depthIndex2,4])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex2,4])*
            (dwB/(dwA+dwB)))*dz2
        NO3LGdata=((grdSTATION.data[julianIndex,depthIndex1,5])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex1,5])*
            (dwB/(dwA+dwB)))*dz1 +((grdSTATION.data[julianIndex,depthIndex2,5])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex2,5])*
            (dwB/(dwA+dwB)))*dz2
        CHLAdata=((grdSTATION.data[julianIndex,depthIndex1,6])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex1,6])*
            (dwB/(dwA+dwB)))*dz1 +((grdSTATION.data[julianIndex,depthIndex2,6])*
            (dwA/(dwA+dwB))+(grdSTATION.data[julianIndex+1,depthIndex2,6])*
            (dwB/(dwA+dwB)))*dz2
        TauX=((grdSTATION.dataXY[julianIndex,0])*
            (dwA/(dwA+dwB))+(grdSTATION.dataXY[julianIndex+1,0])*
            (dwB/(dwA+dwB)))*dz1 +((grdSTATION.dataXY[julianIndex,0])*
      ...
      #  print '---> Minimum %s %f'%("windY", windY.min())
        ...
        return Tdata,Sdata,NH4SMdata,NH4LGdata,NO3SMdata,NO3LGdata,CHLAdata,windX,windY

@memoize
def convertStressToWind(TauX,TauY):
    #DEPRECATED! Use IO.f90 instead
    ...
    windX=np.abs(np.sqrt(np.abs(TauX)/(1.3*1.2e-3)))
    windY=np.abs(np.sqrt(np.abs(TauY)/(1.3*1.2e-3)))
