...
def getStationData(cdf, varlist, time, grdSTATION, log):
    """
    ...
    Nvars=len(varlist)
    var_array_raw=np.zeros((time, int(len(depth)), Nvars),dtype=np.float64)
    var_number=0
    
    for var in varlist:
    
        var_array_raw[:,:,var_number] = cdf.variables[var][:,:]
        ...
        if log is True:
            ...
            print '---> Maximum %s %f'%(var, np.amin(var_array_raw[:,:,int(var_number)]))      
