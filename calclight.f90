...
Module calclight
        ...
        contains
            ...
            ! -------------------------------------------------------------------------------------------
            ! USAGE: call qsw0(radfl0,radmax,cawdir,clouds, real(lat)*pi/180.0,time,daysinyear)
            !
...
!f2py intent(in,overwrite) clouds,lat,time,daysinyear
...
!f2py intent(in,out,overwrite) HEIGHT,SLIG
            ...
            !	  MAXLIG: level of irradiance at midday
                  ...
                  TWLIGHT = 5.76
                    ...
                    DELTA = .3979*SIND(.9856*(D-80)+		&
                        1.9171*(SIND(.9856*D)-.98112))
                            H12 = DELTA*SIND(B*1.)-					&
                          SQRT(1.-DELTA**2)*COSD(B*1.)*COSD(15.*12)

                    HEIGHT = DELTA*SIND(B*1.)-					&
                             SQRT(1.-DELTA**2)*COSD(B*1.)*COSD(15.*H)
